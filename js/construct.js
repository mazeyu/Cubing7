...
function computeCentroids(xx) {
    var f, fl, face;
    for ( f = 0, fl = xx.faces.length; f < fl; f ++ ) {
        face = xx.faces[ f ];
        face.centroid = new THREE.Vector3( 0, 0, 0 );

        face.centroid.add( xx.vertices[ face.a ] );
        face.centroid.add( xx.vertices[ face.b ] );
        face.centroid.add( xx.vertices[ face.c ] );
...
class Tcube {
    ...
    constructor(v, f) {
        ...
        for (let face of f) {
            ...
            let mesh = createMultiMaterialObject(geo, mat);
    ...
    move0(move, gra) {
        ...
        for (let i = 0; i < this.pieces.length; i++) {
            computeCentroids(this.pieces[i].children[0].geometry);
    ...
    addmove(x, y, z, v, move, v2, g) {
        ...
        for (let i = 0; i < this.pieces.length; i++) {
            ...
            this.center = vecAddVec(this.center, centroid);
        }
        ...
        for (let i = 0; i < this.pieces.length; i++) {
            ...
            if (centroid.dot(new THREE.Vector3(x, y, z)) >= v && (v2 === undefined || centroid.dot(new THREE.Vector3(x, y, z)) <= v2)) {
                centroid = vecSubVec(centroid, this.center);
...
function NewCube(n) {
    ...
    cube.addmove(1, 0, 0, thre, 'R', undefined, 4);
    cube.addmove(-1, 0, 0, thre, 'L', undefined,4);
    cube.addmove(0, 0, 1, thre, 'U', undefined,4);
    cube.addmove(0, 0, -1, thre, 'D', undefined,4);
    cube.addmove(0, -1, 0, thre, 'F', undefined,4);
    cube.addmove(0, 1, 0, thre, 'B', undefined,4);
    cube.addmove(1, 0, 0, thre - 1 / n, 'r', undefined,4);
    cube.addmove(-1, 0, 0, thre - 1 / n, 'l', undefined,4);
    cube.addmove(0, 0, 1, thre - 1 / n, 'u', undefined,4);
    cube.addmove(0, 0, -1, thre - 1 / n, 'd', undefined,4);
    cube.addmove(0, -1, 0, thre - 1 / n, 'f', undefined,4);
    cube.addmove(0, 1, 0, thre - 1 / n, 'b', undefined,4);

    cube.addmove(1, 0, 0, thre - 2 / n, '3r', undefined,4);
    cube.addmove(-1, 0, 0, thre - 2 / n, '3l', undefined,4);
    cube.addmove(0, 0, 1, thre - 2 / n, '3u', undefined,4);
    cube.addmove(0, 0, -1, thre - 2 / n, '3d', undefined,4);
    cube.addmove(0, -1, 0, thre - 2 / n, '3f', undefined,4);
    cube.addmove(0, 1, 0, thre - 2 / n, '3b', undefined,4);
...
function displayCube(cube, defaultMoves) {
    ...
    cube.camera.position.set(Math.sin(cube.rotAngle) * 2.5, -Math.cos(cube.rotAngle) * 2.5, 1);
    ...
    controls = new THREE.OrbitControls(cube.camera, cube.renderer.domElement);
